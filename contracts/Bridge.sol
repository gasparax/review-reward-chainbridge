...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    mapping(bytes32 => address) public _resourceIDToSalesAddress;
    mapping(bytes32 => address) public _resourceIDToReviewsAddress;
    ...
    event PermissionEmitted (
        address reviewAddress,
        address recipient
    );

    event DiscountEmitted (
        address salesAddress,
        address recipient
    );

    function setReviewsAddress(address _reviewAddress, bytes32 resourceID) external {
        _resourceIDToReviewsAddress[resourceID] = _reviewAddress;
    }

    function setSalesAddress(address _salesAddress, bytes32 resourceID) external {
        _resourceIDToSalesAddress[resourceID] = _salesAddress;
    }

    function getReviewsAddress(bytes32 resourceID) external view returns (address) {
        return _resourceIDToReviewsAddress[resourceID];
    }

    function getSalesAddress(bytes32 resourceID) external view returns (address) {
        return _resourceIDToSalesAddress[resourceID];
    }

    function updateDiscountPermission(address recipientAddress, uint256 amount, bytes32 resourceID) public {
        address reviewAddress = _resourceIDToReviewsAddress[resourceID];
        address salesAddress = _resourceIDToSalesAddress[resourceID];
        if (reviewAddress != address(0)) {
            IReviews(reviewAddress).setUserPermission(recipientAddress);
            emit PermissionEmitted(reviewAddress,recipientAddress);
        } else {
            ISales(salesAddress).setUserDiscount(recipientAddress, amount);
            emit DiscountEmitted(salesAddress, recipientAddress);
        }
    }
    ...
    function getHandlerByResourceID(bytes32 resourceID)
        public
        view
        returns (address)
    {
    ...
    function executeProposal(
        ...
        updateDiscountPermission(address(recipientAddress), amount, resourceID);
